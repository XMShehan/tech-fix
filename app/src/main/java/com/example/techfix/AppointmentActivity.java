...
public class AppointmentActivity extends AppCompatActivity {
    ...
    private void loadBranches() {
        ...
        try {
            cursor = db.rawQuery("SELECT branchName FROM branches", null);
            while (cursor.moveToNext()) {
                branchNames.add(cursor.getString(
                        cursor.getColumnIndexOrThrow("branchName")));
            }
        } catch (Exception e) {
            Toast.makeText(this, "Unable to load branches", Toast.LENGTH_SHORT).show();
        } finally {
            if (cursor != null) cursor.close();
        }

        if (branchNames.isEmpty()) {
            Toast.makeText(this, "No branches available yet", Toast.LENGTH_SHORT).show();
        }

        ArrayAdapter<String> adapter = new ArrayAdapter<>(
                this, android.R.layout.simple_spinner_item, branchNames);
    ...
    private void detectCurrentLocation() {
        ...
        // Clean up any previous pending listener before starting a new one
        ...
        try {
            ...
            if (lastLocation != null) {
                findNearestBranch(lastLocation.getLatitude(), lastLocation.getLongitude());
                return;
            }
            ...
            if (locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
                locationManager.requestLocationUpdates(
                        LocationManager.GPS_PROVIDER, 1000, 1, activeLocationListener);
            }
            if (locationManager.isProviderEnabled(LocationManager.NETWORK_PROVIDER)) {
                locationManager.requestLocationUpdates(
                        LocationManager.NETWORK_PROVIDER, 1000, 1, activeLocationListener);
            }
    ...
    private void findNearestBranch(double userLatitude, double userLongitude) {
        ...
        try {
            cursor = db.rawQuery("SELECT branchName, latitude, longitude FROM branches", null);
            while (cursor.moveToNext()) {
                String branchName = cursor.getString(cursor.getColumnIndexOrThrow("branchName"));
                double branchLatitude = cursor.getDouble(cursor.getColumnIndexOrThrow("latitude"));
                double branchLongitude = cursor.getDouble(cursor.getColumnIndexOrThrow("longitude"));

                float[] distance = new float[1];
                Location.distanceBetween(userLatitude, userLongitude,
                        branchLatitude, branchLongitude, distance);
            ...
            Toast.makeText(this, "Unable to calculate nearest branch", Toast.LENGTH_SHORT).show();
        } finally {
            if (cursor != null) cursor.close();
        }
        ...
        if (nearestBranch != null) {
            for (int i = 0; i < branchNames.size(); i++) {
                if (branchNames.get(i).equals(nearestBranch)) {
                    spinnerBranch.setSelection(i);
                    break;
                }
            }

            float distanceKm = shortestDistance / 1000;
            Toast.makeText(this,
                    "Nearest Branch: " + nearestBranch
                            + "\nDistance: " + String.format("%.2f", distanceKm) + " km",
                    Toast.LENGTH_LONG).show();
        } else {
            Toast.makeText(this, "No branch data found", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void saveAppointment() {
        String productService = edtProductService.getText().toString().trim();
        String category = edtCategory.getText().toString().trim();
        String priceText = edtPrice.getText().toString().replace("Rs.", "").trim();
        String date = edtDate.getText().toString().trim();
        String time = edtTime.getText().toString().trim();

        if (productService.isEmpty()) {
            edtProductService.setError("Product / Service is required");
            return;
        }
        if (date.isEmpty()) {
            edtDate.setError("Please select a date");
            return;
        }
        if (time.isEmpty()) {
            edtTime.setError("Please select a time");
            return;
        }
        ...
        double price;
        try {
            price = priceText.isEmpty() ? 0 : Double.parseDouble(priceText);
        ...
        values.put("productService", productService);
        ...
        if (result != -1) {
            ...
            Toast.makeText(this, "Failed to create appointment", Toast.LENGTH_LONG).show();
        }
    }
